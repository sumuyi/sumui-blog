	...
	<view class="book-manage">
		...
		<view class="book-list">
			<view v-for="(book) in bookList" :key="book.id" class="book-card">
				<!-- 常用标签 -->
				<view v-if="currentBook.id === book.id" class="tag">已选择</view>

				<!-- 账本信息 -->
				<view class="book-content">
					<image class="book-image" :src="book.coverImage || '../../static/components/image.png'"
						mode="aspectFill"></image>
					<view class="book-info">
						<text class="book-name">{{ book.name }}</text>
						<text class="book-stats">累计:{{ book.billCount || 0 }}笔</text>
						<text class="book-balance">结余:{{ book.balanceAmount || "未设置预算" }}</text>
					</view>
				</view>
...
const editBook = (book) => {
	...
	uni.navigateTo({
		url: `/pages/book/add-edit?id=${book.id}`
	});
};
